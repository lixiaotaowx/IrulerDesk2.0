...
## 第七轮：扬声器开关语义调整（只影响自身播放）

- 背景：视频窗口上方的扬声器按钮与右键头像扬声器应为同一语义，强制控制“本端是否能听到任何声音”，关闭后本端不再播放来自任何人的音频，不应影响对端。

- 改动：
  - 位置： `src/video_components/VideoDisplayWidget.cpp:682–687`
    - `setSpeakerEnabled` 只切换本端 `AudioPlayer` 的播放开关，移除对 `sendViewerListenMute(...)` 的调用。

- 结果：
  - 扬声器关闭 ⇒ 本端硬性静音（谁的声音都听不到）；不改变采集端或其他用户的播放状态。
  - 右键头像扬声器与窗口顶部扬声器保持一致语义。
